...
def parse_file(input_file, langauge_id):
    ...
    try:
        with open(input_file, 'r') as file, open("OutputFromKaufmanParser.txt", 'w') as output:
            ...
            for line in file:
                ...
                #check if parent 
                if not any(line.startswith(prefix) for prefix in prefixes) and not any(substring in line for substring in substrings):
                ...
                if any(line.startswith(prefix) for prefix in prefixes_child):
                ...
                if langauge:
                    langauge = False
                    output.write(f"{category.strip()}: {sub_category}\n{parent_word.strip()}\n{language_id_line.strip()}\n{page_num}\n")
